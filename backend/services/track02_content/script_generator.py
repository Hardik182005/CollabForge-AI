...
class ScriptGenerator:
    ...
    def _fallback_script(self, topic: str, duration: int, style: str) -> dict:
        return {
            ...
            "story": f"Here's what nobody is telling you about {topic} in 2026. "
                     "Early movers are already adapting their strategy around it.",
            "insight": f"The data shows that creators who master {topic} gain a measurable "
                       "competitive edge that compounds over time.",
            "cta": f"Drop a comment below with your biggest {topic} question.",
            "virality_tips": [
    ...
    def _fallback_virality(self, script_text: str, platform: str) -> dict:
        # No LLM available — return an honest unavailable state instead of
        # invented scores/expected views.
        return {
            "status": "unavailable",
            "message": "Virality evaluation needs an LLM provider (OPENAI_API_KEY). No score was generated.",
        }
